...
class Room {
  constructor(code, hostId) {
    ...
    this.players = new Map(); this.destroyTimer = null;
    this.masterRC = null;
  }
  addPlayer(sid, name, role) {
    const c = this.getRoleCounts();
    const lim = { rc: 5, cgo: 5, maquinista: 20 };
    if (c[role] >= lim[role]) return { ok: false, reason: 'Máximo de ' + lim[role] + ' ' + role + ' alcanzado' };
    if (this.destroyTimer) { clearTimeout(this.destroyTimer); this.destroyTimer = null; }
    this.players.set(sid, { id: sid, name, role, joinedAt: Date.now() });
  ...
  removePlayer(sid) {
    ...
    if (sid === this.masterRC) {
      this.masterRC = null;
      for (const [s, pl] of this.players) { if (pl.role === 'rc') { this.masterRC = s; break; } }
    }
  ...
  getPlayerList() {
    return Array.from(this.players.values()).map(p => ({ id: p.id, name: p.name, role: p.role, isMaster: p.id === this.masterRC }));
  }
}
...
io.on('connection', (socket) => {
  ...
  // Only master RC syncs train positions
  socket.on('train:sync', (d) => {
    if (!room || socket.id !== room.masterRC) return;
    socket.to(room.code).volatile.emit('train:sync', d);
  });

  // Maquinista sends commands → broadcast as action
  socket.on('maq:cmd', (d) => {
    if (!room) return;
    const p = room.players.get(socket.id);
    if (!p || p.role !== 'maquinista') return;
    io.to(room.code).emit('action:exec', {
      type: 'trCmd', args: [d.cmd], ctx: { selTrId: d.trainId }, from: p.name + ' (MAQ)'
    });
  });
  ...
  socket.on('disconnect', () => {
    ...
    if (p) {
      ...
      if (room.masterRC) io.to(room.masterRC).emit('master:promoted');
    }
  });
});
