...
// Key fix: CGO/MAQ do NOT run local simulation. Positions come from RC master only.
// RC master uses setInterval backup when tab is hidden.
(function() {
  'use strict';
  ...
  ['openRoute', 'cancelRoute', 'doSpawn', 'mAsp', 'onSw', 'onSig', 'reqBlk', 'grantBlk',
    ...
    'syncAvz', 'updBlkD'].forEach(function(fn) {
  ...
  // ══════════════════════════════════════════════════
  // RC: Override actions + background-safe simulation
  // ══════════════════════════════════════════════════
  if (role === 'rc') {
    ...
    // ── RC BACKGROUND FIX: setInterval keeps simulation running when tab hidden ──
    var bgInterval = null;
    document.addEventListener('visibilitychange', function() {
      if (document.hidden) {
        // Tab hidden → rAF stops → use setInterval as backup
        ...
        // Tab visible again → rAF resumes → stop backup
        if (bgInterval) { clearInterval(bgInterval); bgInterval = null; }
      }
    });

    // ── Master RC: broadcast train + signal state every 250ms ──
    ...
    setInterval(function() {
      ...
      for (var i = 0; i < trains.length; i++) {
        ...
        d.push({
          id: tr.id, pi: tr.pi, t: tr.t, spd: tr.spd,
          x: p ? Math.round((p.x + (q.x - p.x) * tr.t) * 10) / 10 : 0,
          y: p ? Math.round((p.y + (q.y - p.y) * tr.t) * 10) / 10 : 0,
  ...
  // ══════════════════════════════════════════════════
  // CGO & MAQUINISTA: No local simulation, receive only
  // ══════════════════════════════════════════════════
  if (role === 'cgo' || role === 'maquinista') {
    // ── CRITICAL: Disable local train simulation ──
    // This prevents desync. Positions come ONLY from RC master.
    window.tickTrains = function() { /* disabled — positions from RC sync */ };

    // Block all user interactions (but allow via broadcast _bypass)
    ...
    // ── RECEIVE TRAIN SYNC: apply positions from RC master ──
    socket.on('train:sync', function(data) {
      ...
      // Remove trains that RC no longer has
      ...
      if (data.signals && window.S.sig) {
        for (var sigId in data.signals) {
          if (window.S.sig[sigId] && window.S.sig[sigId].asp !== data.signals[sigId].asp) {
            if (typeof window.updAsp === 'function') window.updAsp(sigId, data.signals[sigId].asp);
          }
        }
      }
      ...
      // Maquinista: render cab
      if (role === 'maquinista' && myTrainId) renderCab(data);
      if (role === 'maquinista' && !myTrainId) updateTrainGrid(data.trains);
    });
  ...
  function buildCabView() {
    ...
    sel.innerHTML = '<div style="font:bold 24px Rajdhani,sans-serif;color:#f59e0b">🚂 Selecciona tu tren</div>' +
      '<div style="font:12px \'Share Tech Mono\',monospace;color:#4a6070">El RC debe crear un tren manual primero</div>' +
    ...
    inner.innerHTML =
      ...
      '<div style="height:110px;background:#0d1117;border-top:1px solid #1c2535;display:flex;align-items:center;justify-content:center;gap:24px;padding:12px;">' +
      '<div style="text-align:center;min-width:80px">' +
      '<div style="font:bold 44px Rajdhani,sans-serif;color:#00e87a" id="cabSpeed">0</div>' +
      ...
      '<button onclick="window._maqCmd(\'rev\')" style="padding:8px 16px;font:bold 13px Rajdhani;background:#1a1a40;color:#8888ff;border:1px solid #333366;border-radius:5px;cursor:pointer">↩ Retroceder</button></div>' +
      '<div style="display:flex;align-items:center;gap:6px">' +
      '<div style="font:8px \'Share Tech Mono\',monospace;color:#4a6070">Próxima señal:</div>' +
      '<div id="cabSigAsp" style="width:20px;height:20px;border-radius:50%;background:#222;border:2px solid #333"></div>' +
      '<div id="cabSigName" style="font:10px \'Share Tech Mono\',monospace;color:#8888aa">---</div></div>' +
      '</div>';
    cab.appendChild(inner);
    document.querySelector('.app').appendChild(cab);
  ...
  function resizeCab() { if (!cabCanvas) return; var r = cabCanvas.parentElement; cabCanvas.width = r.clientWidth; cabCanvas.height = r.clientHeight; }

  function updateTrainGrid(trains) {
    var grid = document.getElementById('maqTrainGrid'); if (!grid || !trains || !trains.length) return;
    // Only rebuild if count changed
    if (grid.childElementCount === trains.length) return;
    grid.innerHTML = '';
    for (var i = 0; i < trains.length; i++) {
      var tr = trains[i];
      var card = document.createElement('div');
      card.style.cssText = 'padding:12px 18px;background:#151b25;border:2px solid #1c2535;border-radius:8px;cursor:pointer;text-align:center;min-width:90px;';
      card.innerHTML = '<div style="width:28px;height:10px;background:' + tr.color + ';border-radius:3px;margin:0 auto 5px"></div>' +
  ...
  function renderCab(data) {
    ...
    var nc = sigC[nextSA] || '#222';
    var sa = document.getElementById('cabSigAsp'); sa.style.background = nc; sa.style.borderColor = nc === '#222' ? '#333' : nc;
  }
