...
(function() {
  ...
  ['openRoute','cancelRoute','doSpawn','mAsp','onSw','onSig','reqBlk','grantBlk',
   ...
   'updPN','mallaTrack','drawMalla','syncAvz','updBlkD','updAsp'].forEach(function(fn) {
  ...
  function execB(t, a, c) {
    ...
    try {
      ...
      // When a train is spawned, update maquinista grid
      if (t === 'doSpawn' && role === 'maquinista' && !myTrainId) {
        setTimeout(refreshMaqGrid, 100);
      }
  ...
  function directPosition(train, x, y) {
    if (train.el) train.el.setAttribute('transform', 'translate(' + (x - 27) + ',' + (y - 10) + ')');
    train._sx = x; train._sy = y;
  }
  ...
  if (role === 'rc') {
    ...
    window.spawnTrain = function() { if (_bypass) return O.spawnTrain(); var dir = document.getElementById('tDir') ? document.getElementById('tDir').value : 'AB'; send('doSpawn', [dir]); };
    ...
    window.onSig = function(id) { if (_bypass) return O.onSig(id);
      if (window.S && window.S.mode === 'auto') { if (window.S.pending) { if (window.S.pending === id) { if (window.clearPend) window.clearPend(); return; } var p = window.S.pending; if (window.clearPend) window.clearPend(); send('sigToSig', [p, id]); return; } O.onSig(id); } else { O.onSig(id); } };
    ...
    setInterval(function() {
      ...
      for (var i = 0; i < tr.length; i++) {
        ...
        d.push({ id: t.id, x: p ? Math.round((p.x + (q.x - p.x) * t.t) * 10) / 10 : 0, y: p ? Math.round((p.y + (q.y - p.y) * t.t) * 10) / 10 : 0, spd: t.spd, w: t.w, mHold: t.mHold, sl: t.sl, cs: t.cs, dir: t.dir, stopTimer: t.stopTimer || 0, color: t.color || '#e8a020' });
      }
      ...
      socket.volatile.emit('train:sync', { trains: d, tick: window.S.tick, paused: window.S.paused, simTime: window.simTime || 0, trSpeedMult: window.trSpeedMult || 1, signals: sigs, blks: blks, blkv2: bv2 });
    }, 250);
  }
  ...
  if (role === 'cgo' || role === 'maquinista') {
    ...
    ['openRoute','cancelRoute','doSpawn','mAsp','reqBlk','grantBlk','clearBlk','scenario','resetSim','resetSim2','spawnTrain','runScn','undoLast','trCmd','tPause','setTrSpeed'].forEach(function(fn) {
    ...
    socket.on('train:sync', function(data) {
      ...
      for (var si = 0; si < st.length; si++) {
        ...
        for (var li = 0; li < window.S.trains.length; li++) { if (window.S.trains[li].id === sd.id) { local = window.S.trains[li]; break; } }
        if (!local) continue;
  ...
  function buildCabView() {
    ...
    sel.innerHTML = '<div style="font:bold 24px Rajdhani,sans-serif;color:#f59e0b">🚂 Selecciona tu tren</div>' +
      '<div style="font:12px \'Share Tech Mono\',monospace;color:#4a6070">El RC debe lanzar un tren (Tren Manual → Lanzar)</div>' +
      '<div id="maqTrainGrid" style="display:flex;flex-wrap:wrap;gap:10px;justify-content:center;max-width:600px"></div>' +
      '<div id="maqWaiting" style="font:11px \'Share Tech Mono\',monospace;color:#ffc144">Esperando trenes...</div>';
    cab.appendChild(sel);
    // Cab
    var inner = document.createElement('div'); inner.id = 'cabInner';
    inner.style.cssText = 'flex:1;display:none;flex-direction:column;';
    inner.innerHTML = '<div style="flex:1;position:relative;overflow:hidden;background:#080c14;min-height:200px"><canvas id="cabCanvas" style="width:100%;height:100%;display:block"></canvas></div>' +
      ...
      '<div style="text-align:center;min-width:70px"><div style="font:bold 42px Rajdhani,sans-serif;color:#00e87a" id="cabSpeed">0</div><div style="font:9px \'Share Tech Mono\',monospace;color:#4a6070">km/h</div></div>' +
      '<div style="display:flex;flex-direction:column;gap:2px;min-width:130px"><div style="font:11px \'Share Tech Mono\',monospace;color:#fff" id="cabTrainId">---</div><div style="font:10px \'Share Tech Mono\',monospace;color:#4a6070" id="cabState">---</div><div style="font:10px \'Share Tech Mono\',monospace;color:#4a6070" id="cabPos">---</div></div>' +
      '<div style="display:flex;gap:6px"><button onclick="window._maqCmd(\'go\')" style="padding:8px 16px;font:bold 13px Rajdhani;background:#0e3320;color:#44dd77;border:1px solid #1a5533;border-radius:5px;cursor:pointer">▶ Marcha</button><button onclick="window._maqCmd(\'stop\')" style="padding:8px 16px;font:bold 13px Rajdhani;background:#661616;color:#ff6666;border:1px solid #882222;border-radius:5px;cursor:pointer">⏹ Parar</button><button onclick="window._maqCmd(\'rev\')" style="padding:8px 16px;font:bold 13px Rajdhani;background:#1a1a40;color:#8888ff;border:1px solid #333366;border-radius:5px;cursor:pointer">↩ Retro</button></div>' +
      '<div style="display:flex;align-items:center;gap:6px"><div style="font:8px \'Share Tech Mono\',monospace;color:#4a6070">Señal:</div><div id="cabSigAsp" style="width:18px;height:18px;border-radius:50%;background:#222;border:2px solid #333"></div><div id="cabSigName" style="font:10px \'Share Tech Mono\',monospace;color:#8888aa">---</div></div></div>';
  ...
  function resizeCab() { if (!cabCanvas || !cabCanvas.parentElement) return; cabCanvas.width = cabCanvas.parentElement.clientWidth; cabCanvas.height = cabCanvas.parentElement.clientHeight; }

  // ★ KEY FIX: Build grid from LOCAL S.trains (which are created by doSpawn broadcast)
  var lastGridCount = -1;
  function refreshMaqGrid() {
    ...
    if (!grid) return;
    if (!window.S || !window.S.trains || window.S.trains.length === 0) {
    ...
    if (active.length === 0) { if (waiting) waiting.style.display = ''; return; }
    if (waiting) waiting.style.display = 'none';
    if (active.length === lastGridCount) return;
    lastGridCount = active.length;
    grid.innerHTML = '';
    for (var j = 0; j < active.length; j++) {
      var tr = active[j];
      var card = document.createElement('div');
      card.style.cssText = 'padding:14px 20px;background:#151b25;border:2px solid #1c2535;border-radius:8px;cursor:pointer;text-align:center;min-width:100px;transition:border-color 0.2s;';
      card.innerHTML = '<div style="width:32px;height:12px;background:' + (tr.color || '#e8a020') + ';border-radius:3px;margin:0 auto 6px"></div>' +
      ...
      (function(tid) { card.onclick = function() { myTrainId = tid; document.getElementById('maqSelect').style.display = 'none'; document.getElementById('cabInner').style.display = 'flex'; document.getElementById('cabTrainId').textContent = 'Tren ' + tid; resizeCab(); }; })(tr.id);
  ...
  function renderCab(data) {
    ...
    ctx.fillStyle = my.color || '#f59e0b'; ctx.fillRect(cx - 22, rY - 12, 44, 14); ctx.strokeStyle = '#fff'; ctx.lineWidth = 1.5; ctx.strokeRect(cx - 22, rY - 12, 44, 14); ctx.fillStyle = '#fff'; ctx.font = 'bold 10px Rajdhani'; ctx.textAlign = 'center'; ctx.fillText(my.id, cx, rY - 15); ctx.fillStyle = '#f59e0b'; ctx.font = '14px sans-serif'; ctx.fillText(my.dir === 'AB' ? '→' : '←', cx, rY + 28);
    ...
    var nc = sigC[nextSA] || '#222'; var sa = document.getElementById('cabSigAsp'); sa.style.background = nc; sa.style.borderColor = nc === '#222' ? '#333' : nc;
  }
